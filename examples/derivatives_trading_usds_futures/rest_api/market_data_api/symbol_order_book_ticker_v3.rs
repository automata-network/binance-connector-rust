...
async fn main() -> Result<()> {
    ...
    let params = SymbolOrderBookTickerParams::default();

    let response = rest_client
        .symbol_order_book_ticker(params)
        .await
        .context("symbol_order_book_ticker request failed")?;

    info!(?response.rate_limits, "symbol_order_book_ticker rate limits");
    let data = response.data().await?;
    info!(?data, "symbol_order_book_ticker data");
