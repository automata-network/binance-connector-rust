...
use binance_sdk::derivatives_trading_usds_futures::{
    DerivativesTradingUsdsFuturesRestApi, rest_api::MarkPriceParams,
};
...
async fn main() -> Result<()> {
    ...
    let params = MarkPriceParams::default();

    let response = rest_client
        .mark_price(params)
        .await
        .context("mark_price request failed")?;

    info!(?response.rate_limits, "mark_price rate limits");
    let data = response.data().await?;
    info!(?data, "mark_price data");
