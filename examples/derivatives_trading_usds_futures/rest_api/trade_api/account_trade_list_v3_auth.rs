...
use binance_sdk::derivatives_trading_usds_futures::{
    DerivativesTradingUsdsFuturesRestApi, rest_api::AccountTradeListParams,
};
...
async fn main() -> Result<()> {
    ...
    let params = AccountTradeListParams::builder("DOGEUSDT".to_string()).build()?;

    let response = rest_client
        .account_trade_list(params)
        .await
        .context("account_trade_list request failed")?;

    info!(?response.rate_limits, "account_trade_list rate limits");
    let data = response.data().await?;
    info!(?data, "account_trade_list data");
