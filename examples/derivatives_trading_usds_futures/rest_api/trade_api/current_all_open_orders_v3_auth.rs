...
async fn main() -> Result<()> {
    ...
    let params = CurrentAllOpenOrdersParams::default();

    let response = rest_client
        .current_all_open_orders(params)
        .await
        .context("current_all_open_orders request failed")?;

    info!(?response.rate_limits, "current_all_open_orders rate limits");
    let data = response.data().await?;
    info!(?data, "current_all_open_orders data");
