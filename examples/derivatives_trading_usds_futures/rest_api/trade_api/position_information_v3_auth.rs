...
use binance_sdk::derivatives_trading_usds_futures::{
    DerivativesTradingUsdsFuturesRestApi, rest_api::PositionInformationV3Params,
};
...
async fn main() -> Result<()> {
    ...
    let params = PositionInformationV3Params::default();

    let response = rest_client
        .position_information_v3(params)
        .await
        .context("position_information_v3 request failed")?;

    info!(?response.rate_limits, "position_information_v3 rate limits");
    let data = response.data().await?;
    info!(?data, "position_information_v3 data");
