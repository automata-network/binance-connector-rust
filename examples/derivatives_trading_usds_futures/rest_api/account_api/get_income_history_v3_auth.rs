...
use binance_sdk::derivatives_trading_usds_futures::{
    DerivativesTradingUsdsFuturesRestApi, rest_api::GetIncomeHistoryParams,
};
...
async fn main() -> Result<()> {
    ...
    let params = GetIncomeHistoryParams::default();

    let response = rest_client
        .get_income_history(params)
        .await
        .context("get_income_history request failed")?;

    info!(?response.rate_limits, "get_income_history rate limits");
    let data = response.data().await?;
    info!(?data, "get_income_history data");
