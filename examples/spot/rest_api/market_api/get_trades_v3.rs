...
use binance_sdk::spot::{SpotRestApi, rest_api::GetTradesParams};
...
async fn main() -> Result<()> {
    ...
    let params = GetTradesParams::builder("BNBUSDT".to_string()).build()?;

    let response = rest_client
        .get_trades(params)
        .await
        .context("get_trades request failed")?;

    info!(?response.rate_limits, "get_trades rate limits");
    let data = response.data().await?;
    info!(?data, "get_trades data");
