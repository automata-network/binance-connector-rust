...
use binance_sdk::spot::{SpotRestApi, rest_api::DeleteOrderParams};
...
async fn main() -> Result<()> {
    ...
    let params = DeleteOrderParams::builder("ETHUSDT".to_string())
    ...
    let response = rest_client
        .delete_order(params)
        .await
        .context("delete_order request failed")?;

    info!(?response.rate_limits, "delete_order rate limits");
    let data = response.data().await?;
    info!(?data, "delete_order data");
